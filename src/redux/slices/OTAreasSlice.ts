...
export const fetchOTAreas = createAsyncThunk(
  "OTAreas/fetchOTAreas",
  async (token: any) => {
    try {
      const response = await axios.get(
        `${URLBackend}/api/otareas/listado/?query=01`,
        {
          headers: {
            Authorization: token,
          },
...
const funcionalidadesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchOTAreas.fulfilled, (state, action) => {
...
export const { updateActualArea, updateNextArea } =
  funcionalidadesSlice.actions;
