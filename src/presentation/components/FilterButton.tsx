  ...
  return (
    <div className={`fixed ${className ? className : "top-[4rem] left-5"} z-[13] items-center h-[30vh] transition-all duration-500`}>
      ...
      {filterToggle.value && (
        ...
        className={`${isOT ? "top-[10rem] " : "top-[10rem] "} transition-all duration-500 text-[#f39c12] hover:bg-gray-200  rounded-xl cursor-pointer w-[4rem]  h-[4rem] ${isOT ? "translate-x-[90vw] translate-y-[-5rem]" : "translate-x-[85rem] translate-y-[-5rem]"} `}/>
      )}
    </div>
  );
};
