...
const FilterButton: React.FC<IProps> = React.memo(
  ({ children, isOT, className }) => {
    ...
    return (
      <div
        className={`fixed ${
          className ? className : "top-[4rem] left-5"
        } z-[13] items-center !h-[40vh] transition-all duration-500`}
      >
        <MagnifyingGlassIcon
          className={`bg-[#f39c12] w-[3rem] h-[3rem] text-white p-3 rounded-full shadow-lg top-2 absolute z-30 transition-transform transform cursor-pointer ${
            isHovered.value ? "scale-110 bg-orange-700" : ""
          }`}
        ...
        <div
          ...
          } p-2 items-center  overflow-hidden rounded-md  shadow-2xl`}
        >
          {children}
        </div>
        {filterToggle.value && (
          <RxDoubleArrowLeft
            onClick={() => handleMouseEnter()}
            className={`${
              isOT ? "top-[10rem] " : "top-[10rem] "
            } transition-all duration-500 text-[#f39c12] hover:bg-gray-200  rounded-xl cursor-pointer w-[4rem]  h-[4rem] ${
              isOT
                ? "translate-x-[90vw] translate-y-[-5rem]"
                : "translate-x-[90vw] translate-y-[-5rem]"
            } `}
          />
        )}
