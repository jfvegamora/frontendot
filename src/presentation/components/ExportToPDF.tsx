...
enum EnumPDFDestino {
  ...
  correo = 4,
  remitente_nombre = 5,
  remitente_direccion = 6,
  remitente_telefono = 7,
  remitente_correo = 8,
  observaciones = 9,
}
...
const ExportToPDF: React.FC<IExportToPdf> = ({ rowData }) => {
  ...
  const fetchEtiquetaDespacho = async (destino_id: string) => {
    try {
      ...
      const { data } = await axios(
        `${URLBackend}/api/proyectodestinos/listado/?query=06&_p1=${destino_id}`
      );
        ...
        <Page size={"A4"} style={styles.page}>
          <View style={styles.section}>
            {etiquetaData &&
              ...
              etiquetaData.map((item: any, index: any) => (
                //?TITULO PDF
                <View key={index}>
                  {/* <Text style={styles.title} key={item[0]}>{item[0]}</Text> */}
                  {titleLines.map((line, lineIndex) => (
                    <Text style={styles.title} key={`title-${lineIndex}`}>
                      {line}
                    </Text>
                  ))}
                  <View
                    style={styles.destinatario_section}
                    key={"destino_section"}
                  >
                    <Text
                      style={styles.destinatario_title}
                      key={"Destinatario"}
                    >
                      {"Destinatario:"}
                    </Text>
                    ...
                    {nombreLines.map((line, lindeIndex) => (
                      <Text style={styles.destinatario_campo} key={lindeIndex}>
                        {line}
                      </Text>
                    ))}

                    {direccionLineas.map((line, lineIndex) => (
                      <Text style={styles.destinatario_campo} key={lineIndex}>
                        {line}
                      </Text>
                    ))}

                    <Text
                      style={styles.destinatario_campo}
                      key={item[EnumPDFDestino.correo]}
                    >
                      {item[EnumPDFDestino.correo]}
                    </Text>
                    <Text
                      style={styles.destinatario_campo}
                      key={item[EnumPDFDestino.telefono]}
                    >
                      {item[EnumPDFDestino.telefono]}
                    </Text>
                    <Text
                      style={styles.observaciones}
                      key={item[EnumPDFDestino.observaciones]}
                    >
                      {item[EnumPDFDestino.observaciones]}
                    </Text>
                  </View>

                  <View
                    style={styles.remitente_section}
                    key={"remitente_section"}
                  >
                    <Text style={styles.remitente_title} key={"Remitente"}>
                      {"Remitente:"}
                    </Text>
                    <Text style={styles.remitente_campo} key={item[5]}>
                      {item[5]}
                    </Text>
                    <Text style={styles.remitente_campo} key={item[6]}>
                      {item[6]}
                    </Text>
                    <Text style={styles.remitente_campo} key={item[8]}>
                      {item[8]}
                    </Text>
                    <Text style={styles.remitente_campo} key={item[7]}>
                      {item[7]}
                    </Text>
                  </View>
                </View>
