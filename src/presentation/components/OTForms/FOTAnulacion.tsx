...
const FOTAnulacion: React.FC<IAnulacion> = ({
  ...
  const onSubmit: SubmitHandler<FormData> = async (_jsonData) => {
    try {
      const strUrl = otHistorica
        ? `${URLBackend}/api/othistorica/listado`
        : `${URLBackend}/api/ot/listado`;
      ...
      const query = otHistorica
        ? `?query=05&_pkToDelete=${encodeURIComponent(
            JSON.stringify(pktoDeleteJSON)
          )}`
        : `?query=05&_folio=${_folio}&_estado=${_estado}&_proyecto=${
            codigoProyecto.value
          }&_punto_venta=${
            punto_venta.value
          }&_usuario=${userID}&_origen=${_origen}&_situacion=${
            _jsonData.situacion
          }&_cristalJSONOri=${JSON.stringify(
            cristales
          )}&_armazonJSONOri=${JSON.stringify(armazones)}`;
  ...
  return (
    <div className="useFormContainer useFormDerivacion centered-div use40rem z-30">
      ...
      <form className="userFormulario" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-[95%] flex items-center rowForm">
        ...
        <div className=" w-[93%] flex ml-4 items-center rowForm">
          <div className="w-full">
            <TextInputComponent
              type="text"
              label="Observaciones"
        ...
        <div className="flex justify-center">
          <Button type="submit" className="otActionButton bg-black">
            Anular
          </Button>
        </div>
      </form>
