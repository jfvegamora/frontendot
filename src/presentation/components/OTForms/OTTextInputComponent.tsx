...
const OTTextInputComponent: React.FC<ITextInputProps> = ({
  ...
  const dioptriasState = useAppSelector(
    (store: AppStore) => store.utils?.Dioptrias
  );
  ...
  switch (name) {
    ...
    case "a1_od_esf":
      initialValue = Number.isNaN(dioptrias_receta.value.a1_od.esf)
        ? ""
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (e.target.name) {
      case "a1_od_esf":
        const isESFODPresent = ESFstate.some((subArray: string) =>
          subArray.includes(formatValue)
        );
        if (!isESFODPresent) {
        ...
        break;
      case "a1_od_cil":
        const isCILODPresent = CILstate.some((subArray: string) =>
          subArray.includes(formatValue)
        );
        if (!isCILODPresent) {
        ...
        break;
      case "a1_od_eje":
        const isEJEODPresent = EJEstate.some(
          (subArray: number[]) => subArray[0] === parseFloat(e.target.value)
        );
        if (!isEJEODPresent) {
        ...
        break;
      case "a1_od_ad":
        const isADODPresent = ADstate.some((subArray: string[]) =>
          subArray.includes(formatValue)
        );
        ...
        break;
      case "a1_oi_esf":
        const isESFOIPrsent = ESFstate.some((subArray: string[]) =>
          subArray.includes(formatValue)
        );
        if (!isESFOIPrsent) {
        ...
        break;
      case "a1_oi_cil":
        const isCILOIPresente = CILstate.some((subArray: string[]) =>
          subArray.includes(formatValue)
        );
        if (!isCILOIPresente) {
        ...
        break;
      case "a1_oi_eje":
        const isEJEOIPresent = EJEstate.some(
          (subArray: number[]) => subArray[0] === parseFloat(e.target.value)
        );
        if (!isEJEOIPresent) {
        ...
        break;
      case "a1_oi_ad":
        const isADOIPresent = ADstate.some((subArray: string[]) =>
          subArray.includes(formatValue)
        );
        if (!isADOIPresent) {
  ...
  return (
    <div
      className={` inputStyles flex items-center mb-4 mx-4  relative rounded-xl inputStyles`}
    >
      <Controller
        name={name}
        control={control}
        ...
        render={({ field }) => (
          <div className="flex flex-col  w-full">
            ...
            <Input
              ...
              step={step}
              readOnly={onlyRead}
              className={`${customWidth ? customWidth : "custom-input"} ${
                onlyRead
