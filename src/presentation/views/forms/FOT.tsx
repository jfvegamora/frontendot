...
import {
  A1_ALT, A1_CR_OD, A1_CR_OI, A1_DP, A1_Diametro, A1_GRUPO_OD, A1_GRUPO_OI, A2_CR_OD, A2_CR_OI, A2_DP, A2_Diametro, A2_GRUPO_OD, A2_GRUPO_OI, MODAL, SEXO, TIPO_CLIENTE,
  a1_armazon,
  a1_oi_ad,
  a2_armazon,
  a2_od_cil, a2_od_eje, a2_od_esf, a2_oi_cil, a2_oi_eje, a2_oi_esf, a3_armazon, armazonesJSONsignal, clearArmazonesData, clearDioptrias,
  clearGrupos,
  clearSelectInput,
  codigoProyecto,
  cristalesJSONsignal,
...
const FOT: React.FC<IFOTProps> = ({
  ...
  const insertOT = async (jsonData: any, cristalesJSON: any, armazonesJSON: any) => {
    ...
    let _p1 = `${motivo},${_destino},${estado},"${estado_impresion}","${validar_parametrizacion_id}", "${jsonData.proyecto_codigo || codigoProyecto.value}",${jsonData.establecimiento_id || 1},"${jsonData.cliente_rut || formValues.cliente.cliente_rut}" ,${oftalmologo_id.value || 0} ,"${jsonData.fecha_atencion || fecha_atencion_signal.value}","${jsonData.fecha_entrega_taller || fecha_entrega_taller.value}","${jsonData.fecha_despacho || fecha_despacho.value}","${jsonData.fecha_entrega_cliente || fecha_entrega_cliente.value}",${punto_venta.value || 0},"${numero_receta.value && numero_receta.value !== '' ? numero_receta.value : 0}","${formValues?.receta.fecha_receta ?? ""}",${jsonData.tipo_anteojo_id ?? 0},${typeof dioptrias_receta.value.a1_od.esf === 'string' && !Number.isNaN(dioptrias_receta.value.a1_od.esf) ? dioptrias_receta.value.a1_od.esf : 0},${typeof dioptrias_receta.value.a1_od.cil === 'string' && !Number.isNaN(dioptrias_receta.value.a1_od.cil) ? dioptrias_receta.value.a1_od.cil : 0},${typeof dioptrias_receta.value.a1_od.eje === 'string' && !Number.isNaN(dioptrias_receta.value.a1_od.eje) ? dioptrias_receta.value.a1_od.eje : 0},"${(typeof dioptrias_receta.value.a1_od.ad === 'string' && !Number.isNaN(dioptrias_receta.value.a1_od.ad)) ? dioptrias_receta.value.a1_od.ad : 0}",${typeof dioptrias_receta.value.a1_oi.esf === 'string' && !Number.isNaN(dioptrias_receta.value.a1_oi.esf) ? dioptrias_receta.value.a1_oi.esf : 0},${typeof dioptrias_receta.value.a1_oi.cil === 'string' && !Number.isNaN(dioptrias_receta.value.a1_oi.cil) ? dioptrias_receta.value.a1_oi.cil : 0},${typeof dioptrias_receta.value.a1_oi.eje === 'string' && !Number.isNaN(dioptrias_receta.value.a1_oi.eje) ? dioptrias_receta.value.a1_oi.eje : 0},"${typeof dioptrias_receta.value.a1_oi.ad !== 'string' && !Number.isNaN(dioptrias_receta.value.a1_oi.ad) ? dioptrias_receta.value.a1_oi.ad : 0}",${A1_DP.value !== '' ? A1_DP.value : 0},${formValues["receta"]["a1_alt"] > 0 ? formValues["receta"]["a1_alt"] : 0}, "${A1_GRUPO_OD.value.trim()}" ,"${A1_GRUPO_OI.value.trim()}" ,"${typeof a2_od_esf.value === 'string' ? a2_od_esf.value : 0}","${typeof a2_od_cil.value === 'string' ? a2_od_cil.value : 0}","${typeof a2_od_eje.value === 'string' ? a2_od_eje.value : 0}","${typeof a2_oi_esf.value === 'string' ? a2_oi_esf.value : 0}","${typeof a2_oi_cil.value === 'string' ? a2_oi_cil.value : 0}","${typeof a2_oi_eje.value === 'string' ? a2_oi_eje.value : 0}",${A2_DP.value.trim() || 0},"${A2_GRUPO_OD.value.trim()}","${A2_GRUPO_OI.value.trim()}" ,"${a1_armazon.value.trim() ?? 0}","${a2_armazon.value.trim() ?? 0}","${a3_armazon.value.trim() ?? 0}",${jsonData.cristal1_marca_id || 0},${jsonData.cristal1_diseno_id || 0},${jsonData.cristal1_indice_id || 0},${jsonData.cristal1_material_id || 0},${jsonData.cristal1_tratamiento_id || 0},${jsonData.cristal1_color_id || 0},${(typeof A1_Diametro.value === 'string' && A1_Diametro.value.trim() !== "") ? A1_Diametro.value : 0},"${A1_CR_OD.value.trim() ?? ""}","${A1_CR_OI.value.trim() ?? ""}",${jsonData.cristal1_tratamiento_adicional_id || 0},${jsonData.cristal2_marca_id || 0},${jsonData.cristal2_diseno_id || 0},${jsonData.cristal2_indice_id || 0},${jsonData.cristal2_material_id || 0},${jsonData.cristal2_tratamiento_id || 0},${jsonData.cristal2_color_id || 0},${(typeof A2_Diametro.value === 'string' && A2_Diametro.value.trim() !== "") ? A2_Diametro.value : 0},"${typeof A2_CR_OD.value === 'string' ? A2_CR_OD.value.trim() : ""}","${typeof A2_CR_OI.value === 'string' ? A2_CR_OI.value.trim() : " "}",${jsonData.cristal2_tratamiento_adicional_id || 0},${jsonData.motivo_garantia_id || 0},${jsonData.folio_asociado || 0},${isEditting ? 0 : jsonData.resolucion_garantia_id === 'Aceptada' ? '1' : '2'},"${jsonData.worktracking || 0}","${jsonData.nota_venta || 0}",${jsonData.numero_reporte_firma || 0},"${jsonData.numero_reporte_atencion || 0}","${jsonData.numero_orden_compra || 0}",${jsonData.numero_guia || 0},${jsonData.numero_factura || 0},"${jsonData.folio_interno_mandante || 0}","${jsonData.reporte_interno_mandante || 0}","${jsonData.numero_envio || 0}" ,${jsonData.total || 0},"${formValues?.receta.observaciones || ""}",${rbd_ubicacion},${rbd_cantidad}, ${rep_cantidad},"${estado_validacion}", "${isToggleMontajeValidation.value === true ? 1 : 0}", "${CR1_OD_LAB.value === true ? 2 : 1}" ,"${CR1_OI_LAB.value === true ? 2 : 1}" ,"${CR2_OD_LAB.value === true ? 2 : 1}" ,"${CR2_OI_LAB.value === true ? 2 : 1}"`
  ...
  const clearAllInputsOT = (clearCliente?: boolean, clearOptiLabl?:boolean) => {
    //? A1-OJO-DERECHO
