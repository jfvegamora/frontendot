...
export const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = function (event: IDBVersionChangeEvent) {
      const db = (event.target as IDBOpenDBRequest).result as IDBDatabase;
      const objectStore = db.createObjectStore("reserva_armazones", {
        keyPath: "cod_armazon",
      });
      ...
      const clienteReservaStore = db.createObjectStore(
        "reserva_armazones_beneficiarios",
        { keyPath: "rut_beneficiario" }
      );
      clienteReservaStore.createIndex("rut_beneficiario", "rut_beneficiario", {
        unique: true,
      });
      clienteReservaStore.createIndex("proyecto", "proyecto", {
        unique: false,
      });
...
export const setReservaBeneficiario = (
  db: IDBDatabase,
  jsonData: any,
  userID: string
): Promise<string> => {
...
//?AGREGAR ARMAZONES A DATABASE
export const setArmazones = (
  db: IDBDatabase,
  data: any,
  reservaJSON: any,
  firstTime?: boolean,
  posicionArmazon?: string,
  tipo_anteojo?: string
): Promise<string> => {
  return new Promise((resolve, reject): any => {
    const transaction: IDBTransaction = db.transaction(
    ...
    request.onsuccess = function (event: Event) {
      ...
      if (existingData) {
        ...
        if (existingData["stock_disponible"] <= 0) {
          reject(
            `Armazon ${existingData["cod_armazon"]} sin stock disponible para reservar`
          );
        }
        ...
        const updatedData = {
          ...
          stock_disponible:
            posicionArmazon === "1"
              ? parseInt(existingData.stock_disponible) - 1
              : tipo_anteojo === "3"
              ? parseInt(existingData.stock_disponible) - 1
              : existingData.stock_disponible,
          stock_reservado:
            posicionArmazon === "1"
              ? parseInt(existingData.stock_reservado) + 1
              : tipo_anteojo === "3"
              ? parseInt(existingData.stock_reservado) + 1
              : existingData.stock_reservado,
        };
...
export const clearBaseDatos = (db: IDBDatabase): Promise<string> => {
  return new Promise((resolve, reject) => {
    const transaction_armazones: IDBTransaction = db.transaction(
      ["reserva_armazones"],
      "readwrite"
    );
    const transaction_beneficiarios: IDBTransaction = db.transaction(
      ["reserva_armazones_beneficiarios"],
      "readwrite"
    );

    const objectStore_armazones: IDBObjectStore =
      transaction_armazones.objectStore("reserva_armazones");
    const objectStore_beneficiarios: IDBObjectStore =
      transaction_beneficiarios.objectStore("reserva_armazones_beneficiarios");
    ...
    request_armazones.onerror = function (event: Event) {
      console.error(
        "Error al vaciar la base de datos:",
        (event.target as IDBRequest).error
      );
      reject((event.target as IDBRequest).error);
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event: Event) {
      ...
      if (validateExist) {
        ...
        const missingCodArmazon = codArmazon.filter(
          (cod) => cod !== "" && !data.some((item) => item.cod_armazon === cod)
        );

        let response = {
          value: false,
          missingData: missingCodArmazon,
        };
        resolve(response);
      }

      // const filteredData = data.filter((item) =>
      //   codArmazon.includes(item.cod_armazon)
      // );
      // const missingCodArmazon = codArmazon.filter(
      //   (cod) => !data.some((item) => item.cod_armazon === cod)
      // );
      ...
      // //   missingData: codArmazon
      // //     .filter((cod) => !data.some((item) => item.cod_armazon === cod))
      // //     .join(","),
      // // };
...
export const isExistBeneficiario = (
  db: IDBDatabase,
